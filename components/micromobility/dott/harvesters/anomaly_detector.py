...
class AnomalyHarvester(Harvester):
    def run(self, sources, micromobility_area, dott_daily_prediction, dott_count, **kwargs):
        ...
        time_under_consideration = self.nearest_time(dott_daily_prediction.data, sources[-1].date)
        ...
        last_24_hours_data = self.create_historical_dataframe(dott_count)
  
        comparison_data = pd.DataFrame(dott_daily_prediction.data[time_under_consideration])
