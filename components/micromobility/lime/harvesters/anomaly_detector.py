...
class AnomalyHarvester(Harvester):
    def run(self, sources, micromobility_area, lime_daily_prediction, lime_count, **kwargs):
        ...
        time_under_consideration = self.nearest_time(lime_daily_prediction.data, sources[-1].date)
        ...
        last_24_hours_data = self.create_historical_dataframe(lime_count)
  
        comparison_data = pd.DataFrame(lime_daily_prediction.data[time_under_consideration])
