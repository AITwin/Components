...
class AnomalyHarvester(Harvester):
    def run(self, sources, micromobility_area, pony_daily_prediction, pony_count, **kwargs):
        ...
        time_under_consideration = self.nearest_time(pony_daily_prediction.data, sources[-1].date)
        ...
        last_24_hours_data = self.create_historical_dataframe(pony_count)
  
        comparison_data = pd.DataFrame(pony_daily_prediction.data[time_under_consideration])
