...
def main():
    ...
    if not features:
        raise SystemExit("No Bolt vehicles found.")
    ...
    if available.empty:
        raise SystemExit("No available Bolt vehicles.")
    ...
    cmap = linear.Greens_09.scale(vmin, vmax)
    ...
    for _, row in available.iterrows():
        ...
        color = cmap(range_km)
        label = f"Bolt Scooter — {range_km:.1f} km"
    ...
    m.save("bolt_micromobility_vehicles.html")
    print(f"✅ Saved bolt_micromobility_vehicles.html | Available vehicles: {len(available)}")
