...
def main():
    ...
    try:
        ...
        print(f"⚠️  Dott micromobility API timeout or connection error. Service may be slow or unavailable.")
        return
    if not features:
        raise SystemExit("No Dott vehicles found.")
    ...
    if available.empty:
        raise SystemExit("No available Dott vehicles.")

    # Calculate range in km
    available["range_km"] = pd.to_numeric(available.get("current_range_meters", 0), errors="coerce") / 1000.0
    ...
    for _, row in available.iterrows():
        ...
        color = cmap(range_km)
        label = f"Dott Scooter — {range_km:.1f} km"
    ...
    m.save("dott_vehicles.html")
    print(f"✅ Saved dott_vehicles.html | Available vehicles: {len(available)}")
